...
- **Policy engine.** Every transaction the bot submits is deserialized and inspected before signing:
  ...
  - SPL token and native-SOL instruction inspection — `SetAuthority` is always rejected; SPL transfer destinations must be whitelisted, be an ATA of a whitelisted owner, or occur inside a DEX transaction; a standalone `SystemProgram.transfer` must have a whitelisted recipient or occur inside a DEX transaction;
  - transaction simulation with CPI discovery — programs invoked indirectly must also pass the allowlist.
- **Trust boundary.** Compromise of the bot process does not yield the key, and unknown programs, `SetAuthority`, and standalone transfers to unvouched addresses are never signed — but a transfer or approve **bundled with an allowlisted DEX instruction is signed**, a deliberate compatibility trade-off (WSOL wraps, intermediate-account funding) that a fully compromised bot could abuse to exfiltrate, so containment is partial by design. The model does _not_ defend against compromise of the host itself or of an allowlisted on-chain program.

This is the same problem shape as permission systems for autonomous AI agents: an untrusted automated process, a policy layer with hard guarantees, and a human-in-the-loop unlock step. A **Rust rewrite of the signer** lives in [`signer-rs/`](signer-rs/): a drop-in daemon verified byte-for-byte against the TypeScript implementation by golden vectors and a differential harness, fixing security gaps found during the port (frame-size cap, pipelined-frame stall, unenforced SPL `Approve`, unchecked native-SOL transfers, key zeroization, unlock rate limiting, opt-in socket peer authentication). See [signer-rs/README.md](signer-rs/README.md) for the design write-up.
